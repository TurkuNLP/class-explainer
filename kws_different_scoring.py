...
THRESHOLD = 5
SAVE_N = 300
SAVE_FILE = "testi_stable_keywords.tsv"
UNSTABLE_FILE = "testi_unstable_keywords.tsv"
...
def class_frequencies(data):
    ...
    b = data.groupby(['token','pred_label'])['score'].sum()
    b2 = data.groupby(['token','pred_label'])['score'].std(ddof=0)

    ##for key, number in {'HI': 0, 'ID':1, 'IN':2,'IP':3,'LY':4,'NA':5,'OP':6,'SP':7}.items():
    #   number_of_docs_in_this_class = len(data.groupby(['pred_label', 'source'])['document_id'].unique())
        

    c = data.groupby(['token','pred_label'])['source'].unique()
    
    return pd.concat([a.to_frame(), b.to_frame().add_suffix("_sum"), b2.to_frame().add_suffix("_std"), c.to_frame()],axis=1)         
...
def filter_class_df(data, key, number):
    ...
    Save to a file.
    """
    with open('../../samuel/class-explainer/class_df.json') as f:   #
        dataset = json.load(f)
...
if __name__=="__main__":
    ...
    #print(df_full[df_full.document_id =='doc_0'])
    ...
    # mouse       3       [0.2]        [file2]
    # turns into
    # token       label   score            score_sum   score_std     source
    # mouse       5       [0.5, 0.4]       [0.9]        [0.05]        [file2, file4]
    ...
    # calculate score mean from the score_sum

    num_of_docs_with_this_label = []
    for i in range(0,25):
        num_of_docs_with_this_label.append(len(df_full[df_full.pred_label == i]['document_id']))
    
    score_mean = []
    
    for index, row in freq_array.iterrows():
        label = int(index[1])
        num_of_docs = int(num_of_docs_with_this_label[label])
        score_mean.append(row.score_sum/num_of_docs)

    freq_array['score_mean'] = score_mean
    ...
    new_df_save.drop(['score','score_sum','source'], axis=1, inplace=True)
    df_unstable.drop(['score','score_sum','source'], axis=1, inplace=True)
